...
def get_sum_of_overlaps(input_list: list) -> int:
    sum = 0
    for line in input_list:
        first, second = line
        if int(second[0]) in range(int(first[0]), int(first[1]) + 1):
            sum += 1
        elif int(second[1]) in range(int(first[0]), int(first[1]) + 1):
            sum += 1
        elif int(first[0]) in range(int(second[0]), int(second[1]) + 1):
            sum += 1
...
if __name__ == "__main__":
    ...
    if input is not None:
        print(get_sum_of_overlaps(input))
